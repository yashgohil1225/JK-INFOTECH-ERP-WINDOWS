...
' 1. Start PostgreSQL & Redis services if registered
WshShell.Run "cmd.exe /c net start JK_Infotech_PostgreSQL", 0, True
WshShell.Run "cmd.exe /c net start JK_Infotech_Redis", 0, True
...
' 5. Launch Desktop Client Interface SILENTLY (No pop-up error dialogs)
strPsLaunch = "powershell.exe -WindowStyle Hidden -Command ""$pkg = Get-AppxPackage *9428b0f2-9cad-4953-a4b8-da3e6a84d40a* | Select-Object -First 1; if ($pkg) { start ('shell:AppsFolder\' + $pkg.PackageFamilyName + '!App') }"""
WshShell.Run strPsLaunch, 0, False