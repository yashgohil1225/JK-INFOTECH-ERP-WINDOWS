...
async def main():
    print("Testing full invoice Jinja template rendering...")
    ...
    print(f"Jinja rendered HTML size: {len(rendered_html)} bytes.")
    
    # Render PDF using Playwright engine
    pdf_bytes = await _generate_pdf_async(rendered_html)
    out_pdf = os.path.join(os.path.dirname(__file__), "invoice_test_result.pdf")
    with open(out_pdf, "wb") as f:
        f.write(pdf_bytes)
        
    print(f"[SUCCESS] Full Tax Invoice PDF generated successfully! Saved to: {out_pdf} ({len(pdf_bytes)} bytes)")
