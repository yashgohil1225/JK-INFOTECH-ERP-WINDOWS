...
# This is the fastest, sub-100ms UWP activation method supported by Windows 10 & 11
try {
    Log "Attempting instant launch via jkerpwindows: protocol..."
...
# Ensure UWP AppContainer loopback exemption is granted if protocol launch failed
try {
    $pkg = Get-AppxPackage -Name "*$appName*" -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($pkg) {
        CheckNetIsolation.exe LoopbackExempt -a "-n=$($pkg.PackageFamilyName)" 2>$null | Out-Null
    }
} catch {}
