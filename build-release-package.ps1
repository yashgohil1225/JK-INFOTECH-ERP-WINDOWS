...
Write-Host "JK INFOTECH ERP v1.1.5 — Full Release Packaging" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
...
# ---------------------------------------------------------------------
Write-Host "`n[2/4] Building Windows UWP Client (Release Mode v1.1.5.0)..." -ForegroundColor Yellow
...
try {
    ...
    $appxDir = Join-Path $frontendDir "windows\AppPackages\JKErpWindows\JKErpWindows_1.1.5.0_x64_Test"
    if (-not (Test-Path $appxDir)) {
        # Fallback check if output directory name differs slightly
        $appxDirFallback = Get-ChildItem (Join-Path $frontendDir "windows\AppPackages\JKErpWindows") | Where-Object { $_.Name -like "*1.1.5*" } | Select-Object -First 1
        if ($appxDirFallback) {
            $appxDir = $appxDirFallback.FullName
        } else {
            Write-Warning "AppPackages v1.1.5 folder not found automatically. Using available package."
        }
    }
...
$setupExePath = Join-Path $outputDir "JK_Infotech_ERP_Setup_v1.1.5.exe"
...
$zipPath = Join-Path $outputDir "JK_Infotech_ERP_v1.1.5.zip"
$updatesZipPath = Join-Path $WorkspaceRoot "updates\JK_Infotech_ERP_v1.1.5.zip"
